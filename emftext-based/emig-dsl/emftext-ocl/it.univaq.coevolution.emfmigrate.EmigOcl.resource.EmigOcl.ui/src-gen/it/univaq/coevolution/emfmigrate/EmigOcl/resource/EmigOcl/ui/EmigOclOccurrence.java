 ...
 */
public class EmigOclOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.occurences.declaration";
	...
	private it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.EmigOclTokenScanner tokenScanner;
	...
	private it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.IEmigOclTextResource textResource;
	 ...
	 */
	public EmigOclOccurrence(it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.IEmigOclTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.EmigOclTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.mopp.EmigOclMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.EmigOclBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.EmigOclPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.EmigOclPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.EmigOclPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.EmigOclPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.EmigOclOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.ui.EmigOclOccurrence.DECLARATION_ANNOTATION_ID);
	}
