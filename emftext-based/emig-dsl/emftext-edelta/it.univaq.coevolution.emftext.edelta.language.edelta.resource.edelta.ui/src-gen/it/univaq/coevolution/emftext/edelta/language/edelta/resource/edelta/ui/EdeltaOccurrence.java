 ...
 */
public class EdeltaOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.occurences.declaration";
	...
	private it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.EdeltaTokenScanner tokenScanner;
	 ...
	 */
	public EdeltaOccurrence(it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.EdeltaTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.mopp.EdeltaMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.EdeltaBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.EdeltaPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.EdeltaPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.EdeltaPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.EdeltaPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.EdeltaOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.ui.EdeltaOccurrence.DECLARATION_ANNOTATION_ID);
	}
