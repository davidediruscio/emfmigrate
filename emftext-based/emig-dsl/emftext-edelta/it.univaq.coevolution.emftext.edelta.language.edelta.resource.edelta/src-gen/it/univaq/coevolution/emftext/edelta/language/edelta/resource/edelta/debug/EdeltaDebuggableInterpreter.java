 ...
 */
public class EdeltaDebuggableInterpreter<ResultType, ContextType> extends it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.debug.AbstractEdeltaDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.AbstractEdeltaInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public EdeltaDebuggableInterpreter(it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.AbstractEdeltaInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.EdeltaStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.AbstractEdeltaInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
		org.eclipse.emf.ecore.resource.Resource resource = element.eResource();
		if (resource instanceof it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaTextResource) {
			it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaTextResource textResource = (it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaTextResource) resource;
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.debug.EEdeltaDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.EdeltaEObjectUtil.getDepth(current);
		stopCondition = new it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.EdeltaEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.EdeltaEObjectUtil.getDepth(current) - 1;
		stopCondition = new it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.IEdeltaCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
