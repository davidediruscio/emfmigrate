 ...
 */
public class EdeltaMinimalModelHelper {
	
	private final static it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.EdeltaEClassUtil eClassUtil = new it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.EdeltaEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.EdeltaStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.EdeltaListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.EdeltaStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = it.univaq.coevolution.emftext.edelta.language.edelta.resource.edelta.util.EdeltaListUtil.castListUnchecked(value);
