 ...
 * </ul>
 */
public class EmigBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.mopp.EmigMetaInformation().getSyntaxName();
	
	private it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public EmigBracketPreferencePage() {
		super();
		
		it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigMetaInformation metaInformation = new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.mopp.EmigMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigEditor) {
			((it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigEditor) editor).invalidateTextRepresentation();
		}
	}
