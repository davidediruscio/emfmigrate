 ...
 */
public class EmigOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.occurences.declaration";
	...
	private it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigTokenScanner tokenScanner;
	...
	private it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigTextResource textResource;
	 ...
	 */
	public EmigOccurrence(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.mopp.EmigMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.ui.EmigOccurrence.DECLARATION_ANNOTATION_ID);
	}
