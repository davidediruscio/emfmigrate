 ...
 */
public class EmigDebuggableInterpreter<ResultType, ContextType> extends it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.AbstractEmigDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.AbstractEmigInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public EmigDebuggableInterpreter(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.AbstractEmigInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.AbstractEmigInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
		org.eclipse.emf.ecore.resource.Resource resource = element.eResource();
		if (resource instanceof it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigTextResource) {
			it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigTextResource textResource = (it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigTextResource) resource;
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EEmigDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigEObjectUtil.getDepth(current);
		stopCondition = new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigEObjectUtil.getDepth(current) - 1;
		stopCondition = new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.IEmigCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
