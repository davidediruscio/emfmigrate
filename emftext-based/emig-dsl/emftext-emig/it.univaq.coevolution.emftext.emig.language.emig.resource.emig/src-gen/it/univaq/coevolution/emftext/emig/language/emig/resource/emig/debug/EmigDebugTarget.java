...
public class EmigDebugTarget extends it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugElement implements org.eclipse.debug.core.model.IDebugTarget, it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.IEmigDebugEventListener {
	
	/**
	 * associated system process
	 */
	private it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugProcess process;
	...
	private it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.IEmigDebugEventListener> eventListeners = new java.util.ArrayList<it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.IEmigDebugEventListener>();
	
	private it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.mopp.EmigMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.IEmigDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public EmigDebugTarget(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.IEmigDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.IEmigDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.mopp.EmigMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigLineBreakpoint lineBreakpoint = (it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigLineBreakpoint) breakpoint;
	...
	public it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugProxy getDebugProxy() {
