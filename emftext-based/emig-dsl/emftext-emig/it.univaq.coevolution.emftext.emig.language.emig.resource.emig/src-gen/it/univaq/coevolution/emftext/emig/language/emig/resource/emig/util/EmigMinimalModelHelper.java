 ...
 */
public class EmigMinimalModelHelper {
	
	private final static it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigEClassUtil eClassUtil = new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigListUtil.castListUnchecked(value);
