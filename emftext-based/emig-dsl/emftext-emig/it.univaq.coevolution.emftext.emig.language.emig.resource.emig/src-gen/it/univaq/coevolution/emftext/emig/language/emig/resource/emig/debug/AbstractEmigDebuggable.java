...
public abstract class AbstractEmigDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			it.univaq.coevolution.emftext.emig.language.emig.resource.emig.mopp.EmigPlugin.logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			it.univaq.coevolution.emftext.emig.language.emig.resource.emig.mopp.EmigPlugin.logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EEmigDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugMessage message = new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EmigDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EEmigDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EEmigDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new it.univaq.coevolution.emftext.emig.language.emig.resource.emig.util.EmigPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EEmigDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(it.univaq.coevolution.emftext.emig.language.emig.resource.emig.debug.EEmigDebugMessageTypes.STARTED, true);
			setSuspend(true);
