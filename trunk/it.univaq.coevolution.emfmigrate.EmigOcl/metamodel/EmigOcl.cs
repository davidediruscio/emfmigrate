...
RULES {
	Module ::= "module" name[] ";" (!0 "models" models ("," models)* ";")? (!0 features*)?;
	...
	@Operator(type="unary_postfix", weight="11", superclass="OclExpression")
	...
	OperatorCallExp ::= source operationName[BOOLOP] arguments;

	@Operator(type="binary_left_associative", weight="2", superclass="OclExpression")
	RelOpCallExp ::= source operationName[RELOP] arguments;

	@Operator(type="binary_left_associative", weight="3", superclass="OclExpression")
	AddOpCallExp ::= source operationName[ADDOP] arguments;

	@Operator(type="binary_left_associative", weight="4", superclass="OclExpression")
	IntOpCallExp ::= source operationName[INTOP] arguments;

	@Operator(type="binary_left_associative", weight="5", superclass="OclExpression")
	MulOpCallExp ::= source operationName[MULOP] arguments;

	@Operator(type="unary_prefix", weight="6", superclass="OclExpression")
	NotOpCallExp ::= operationName[NOTOP] source;

	@Operator(type="primitive", weight="20", superclass="OclExpression")
	BraceExp ::= "(" source ")";

	@Operator(type="unary_postfix", weight="12", superclass="OclExpression")
	CollectionOperationCallExp ::= source "->" operationName[] "(" (arguments ("," arguments)*)? ")";

	@Operator(type="unary_postfix", weight="14", superclass="OclExpression")
	IterateExp ::= source "->" "iterate" "(" iterators ("," iterators)* ";" result "|" body ")";

	@Operator(type="unary_postfix", weight="13", superclass="OclExpression")
