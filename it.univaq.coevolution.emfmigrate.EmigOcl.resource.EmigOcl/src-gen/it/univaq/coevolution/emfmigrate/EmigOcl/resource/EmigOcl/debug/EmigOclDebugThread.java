...
public class EmigOclDebugThread extends it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.debug.EmigOclDebugElement implements org.eclipse.debug.core.model.IThread, it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.debug.IEmigOclDebugEventListener {
	
	private boolean suspended = false;
	private it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.debug.EmigOclDebugTarget debugTarget;
	
	public EmigOclDebugThread(it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.debug.EmigOclDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.debug.EmigOclDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.util.EmigOclStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.debug.EmigOclStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public it.univaq.coevolution.emfmigrate.EmigOcl.resource.EmigOcl.debug.EmigOclDebugTarget getTarget() {
		return debugTarget;
	}
